...
export default function AdminChecklistManager() {
    ...
    const [mainTab, setMainTab] = useState<'management' | 'performance' | 'ranking'>('management')
    ...
    const handleDuplicate = async (id: string) => {
        ...
        setIsProcessing(true)
        const res = await duplicateChecklistTemplateAction(id)
        if (res.success) {
            fetchTemplates()
            handleSelectTemplate(res.data)
        }
    ...
    const handleDelete = async (id: string) => {
        if (!confirm('Deseja EXCLUIR permanentemente este template?')) return
        setIsProcessing(true)
        const res = await deleteChecklistTemplateAction(id)
        if (res.success) {
            ...
            <div className="flex gap-2 p-1 bg-gray-100 rounded-2xl w-fit">
                <button 
                    ...
                    className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                        mainTab === 'management' ? 'bg-white text-[#B13A2B] shadow-sm' : 'text-gray-400 hover:text-gray-600'
                    }`}
                >
                ...
                <button 
                    ...
                    className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                        mainTab === 'performance' ? 'bg-white text-[#B13A2B] shadow-sm' : 'text-gray-400 hover:text-gray-600'
                    }`}
                >
                ...
                <button 
                    ...
                    className={`flex items-center gap-2 px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                        mainTab === 'ranking' ? 'bg-white text-[#B13A2B] shadow-sm' : 'text-gray-400 hover:text-gray-600'
                    }`}
                >
                                ...
                                <div className="bg-white rounded-[32px] p-8 border border-gray-200 shadow-sm relative overflow-hidden">
                                    <div className="flex justify-between items-start mb-6">
                                        ...
                                        <div className="flex gap-2">
                                            <button 
                                                ...
                                                className="p-2.5 bg-gray-50 text-gray-400 rounded-xl hover:text-gray-900 hover:bg-gray-100 transition"
                                            ...
                                            <button 
                                                ...
                                                className="p-2.5 bg-gray-50 text-gray-400 rounded-xl hover:text-red-600 hover:bg-red-50 transition"
                                    ...
                                    {activeTab === 'config' ? (
                                        <div className="space-y-6">
                                            ...
                                            <div className="flex flex-wrap items-center gap-6 p-5 bg-gray-50 rounded-2xl border border-gray-100">
                                                <label className="flex items-center gap-3 cursor-pointer group">
                                                    <input 
                                                        type="checkbox"
                                                        checked={selectedTemplate.active}
                                                        onChange={(e) => setSelectedTemplate({...selectedTemplate, active: e.target.checked})}
                                                        className="w-4 h-4 rounded text-[#B13A2B] focus:ring-[#B13A2B] bg-white border-gray-200"
                                                    />
                                                    <div className="flex flex-col">
                                                        <span className="text-[10px] font-black text-gray-900 uppercase tracking-widest">Ativo Globalmente</span>
                                                        <span className="text-[8px] font-bold text-gray-400 uppercase">Disponível para agendamento</span>
                                                    </div>
                                                </label>
                                                ...
                                                <label className="flex items-center gap-3 cursor-pointer group">
                                                    ...
                                                    />
                                                    <div className="flex flex-col">
                                                        <span className="text-[10px] font-black text-gray-900 uppercase tracking-widest">Exigir Assinatura</span>
                                                        <span className="text-[8px] font-bold text-gray-400 uppercase">Nome e Cargo no final</span>
                                                    </div>
                                                </label>
                                                ...
                                                <div className="flex items-center gap-3">
                                                    <div className="flex flex-col">
                                                        <span className="text-[10px] font-black text-gray-900 uppercase tracking-widest">Ordem de Exibição</span>
                                                        <span className="text-[8px] font-bold text-gray-400 uppercase">Prioridade visual</span>
                                                    </div>
                                                    <input 
                                                        type="number"
                                                        className="w-16 bg-white border border-gray-200 rounded-xl px-3 py-1 text-xs font-bold text-gray-700"
                                                        value={0}
                                                        onChange={() => {}}
                                                    />
                                                </div>

...
function LastCountContext({ session, onHistory }: { session: { id: string, completedAt: string }, onHistory: () => void }) {
    ...
    return (
        <div className="bg-white border border-gray-100 rounded-[24px] p-4 flex items-center justify-between shadow-sm">
            <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-gray-50 flex items-center justify-center">
                    <Clock className="w-5 h-5 text-gray-400" />
                </div>
                <div>
                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Última contagem finalizada</p>
                    <p className="text-sm font-black text-gray-900">há {formatHours(ageH)}</p>
                </div>
            </div>
            <button
                onClick={() => onHistory()}

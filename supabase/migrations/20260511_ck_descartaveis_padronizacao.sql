...
BEGIN
    ...
    SELECT id INTO v_group_id FROM count_groups WHERE name = 'CK — Descartáveis' LIMIT 1;
    ...
    IF NOT EXISTS (SELECT 1 FROM count_items WHERE group_id = v_group_id AND name = 'POTE 145ML - 25UN') THEN
        INSERT INTO count_items (group_id, name, unit, is_active) VALUES (v_group_id, 'POTE 145ML - 25UN', 'UN', true);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM count_items WHERE group_id = v_group_id AND name = 'POTE 30ML - 50UN') THEN
        INSERT INTO count_items (group_id, name, unit, is_active) VALUES (v_group_id, 'POTE 30ML - 50UN', 'UN', true);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM count_items WHERE group_id = v_group_id AND name = 'POTE 60ML - 100UN') THEN
        INSERT INTO count_items (group_id, name, unit, is_active) VALUES (v_group_id, 'POTE 60ML - 100UN', 'UN', true);
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM count_items WHERE group_id = v_group_id AND name = 'QUENTINHA 1100ML - 100UN') THEN
        INSERT INTO count_items (group_id, name, unit, is_active) VALUES (v_group_id, 'QUENTINHA 1100ML - 100UN', 'PCT', true);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM count_items WHERE group_id = v_group_id AND name = 'QUENTINHA 220ML - 100UN') THEN
        INSERT INTO count_items (group_id, name, unit, is_active) VALUES (v_group_id, 'QUENTINHA 220ML - 100UN', 'PCT', true);
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM count_items WHERE group_id = v_group_id AND name = 'SALADEIRA 1000ML') THEN
        INSERT INTO count_items (group_id, name, unit, is_active) VALUES (v_group_id, 'SALADEIRA 1000ML', 'UN', true);
    END IF;

    -- =========================================================================
    -- 7. Incluir embalagens de executivo
    -- =========================================================================
    IF NOT EXISTS (SELECT 1 FROM count_items WHERE group_id = v_group_id AND name = 'EMBALAGEM TAMPA EXECUTIVO - 100UN') THEN
        INSERT INTO count_items (group_id, name, unit, unit_observation, is_active) 
        VALUES (v_group_id, 'EMBALAGEM TAMPA EXECUTIVO - 100UN', 'PCT', 'Contar pacote fechado com 100 unidades.', true);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM count_items WHERE group_id = v_group_id AND name = 'EMBALAGEM DE ISOPOR EXECUTIVO - 100UN') THEN
        INSERT INTO count_items (group_id, name, unit, unit_observation, is_active) 
